...
export function parseNaturalLanguage(text) {
  ...
  if (/(?:1|一)\s*色|黑白|單色/.test(source)) result.innerColorCount = 1;
  if (/(?:4|四)\s*色|彩色|四色|全彩/.test(source)) result.innerColorCount = 4;
  ...
  if (noFinishing) {
    ...
    if (/亮膜|上亮|亮p/i.test(source)) selectedFinishing.push('上亮膜');
    if (/霧膜|霧面|上霧|霧p/i.test(source)) selectedFinishing.push('上霧膜');
    if (/局部上光|局部光|局光|上光/.test(source)) selectedFinishing.push('局部上光');
    if (/燙金|燙銀|燙色|燙字/.test(source)) selectedFinishing.push('燙金');
